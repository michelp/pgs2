Datum S2LatLngRect(PG_FUNCTION_ARGS) {
  pgs2_S2LatLngRect *l;
  pgs2_S2LatLng *lo, *hi;
...
Datum S2LatLngRect_in(PG_FUNCTION_ARGS) {
  bool has_delim;
  pgs2_S2LatLngRect *l;
  ...
  if (*input != '\0')
    ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                    errmsg("no null byte at end of %s: \"%s\"", "S2LatLngRect",
                           orig_string)));

  PGS2_RETURN_S2LATLNGRECT_P(l);
}

Datum S2LatLngRect_out(PG_FUNCTION_ARGS) {
...
Datum S2LatLngRect_eq(PG_FUNCTION_ARGS) {
