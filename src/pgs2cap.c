...
Datum S2Cap_in(PG_FUNCTION_ARGS) {
  bool has_delim;
  pgs2_S2Cap *cap;
  ...
  if (*input++ != DELIM)
    ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                    errmsg("invalid delimiter after x in %s: \"%s\"", "S2Cap",
                           orig_string)));
  ...
  if (*input++ != DELIM)
    ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                    errmsg("invalid delimiter after y in %s: \"%s\"", "S2Cap",
                           orig_string)));
  ...
  if (*input++ != COLON)
    ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                    errmsg("invalid delimiter after z in %s: \"%s\"", "S2Cap",
                           orig_string)));
  ...
  if (has_delim) {
    if (*input++ != RPAREN)
      ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                      errmsg("invalid ending delimter for %s: \"%s\"", "S2Cap",
                             orig_string)));
    while (isspace((unsigned char)*input))
      input++;
  }
  ...
  if (*input != '\0')
    ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                    errmsg("no null byte at end of %s: \"%s\"", "S2Cap",
                           orig_string)));

  PGS2_RETURN_S2CAP_P(cap);
}

Datum S2Cap_out(PG_FUNCTION_ARGS) {
...
Datum S2Cap_eq(PG_FUNCTION_ARGS) {
  pgs2_S2Cap *A, *B;
  A = PGS2_GETARG_S2CAP_P(0);
  B = PGS2_GETARG_S2CAP_P(1);
  PG_RETURN_BOOL(A->center.x == B->center.x && A->center.y == B->center.y &&
